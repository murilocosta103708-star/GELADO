```md
# 🧊 Gelado Intelligence

<p align="center">
  <img src="https://img.shields.io/badge/Status-Em%20Desenvolvimento-blue">
  <img src="https://img.shields.io/badge/BI-Industrial-success">
  <img src="https://img.shields.io/badge/Logística-Inteligente-orange">
  <img src="https://img.shields.io/badge/Data-Analytics-purple">
</p>

---

# 📌 Sobre o Projeto

O **Gelado Intelligence** é um projeto de Business Intelligence voltado para empresas de distribuição de gelo, focado em:

- Controle operacional
- Eficiência logística
- Gestão financeira
- Inteligência industrial
- Monitoramento de perdas
- Análise de rentabilidade

O sistema transforma dados operacionais em indicadores estratégicos para auxiliar na tomada de decisões.

---

# 🎯 Objetivos Estratégicos

## ⚡ Otimização Energética
Monitorar consumo energético e eficiência produtiva.

## 🚚 Eficiência Logística
Melhorar ocupação da frota e reduzir custos operacionais.

## 💰 Rentabilidade Real
Identificar margem de lucro por cliente, região e canal.

## ❄️ Controle de Perdas
Reduzir desperdícios e derretimento durante transporte e armazenagem.

---

# 📊 KPIs do Projeto

| KPI | Descrição |
|------|------------|
| OEE | Eficiência Global dos Equipamentos |
| CPK | Custo por Quilo Produzido |
| Drop Size | Volume Médio por Entrega |
| Shrinkage | Taxa de Perda Operacional |
| EBITDA | Resultado Operacional |
| Margem de Contribuição | Lucro por Entrega |

---

# 📈 Dashboard Estratégico

## 🏭 Produção
- Controle produtivo
- Consumo energético
- Eficiência industrial
- Capacidade ociosa

## 🚛 Logística
- Rotas
- Custos de entrega
- Volume transportado
- Ocupação da frota

## 💵 Financeiro
- Receita
- Custos
- EBITDA
- Margem operacional

---

# 📋 Perguntas Respondidas

O sistema responde questões importantes como:

- Quanto foi faturado hoje?
- Qual produto mais vendido?
- Qual cliente gera maior lucro?
- Quanto gelo foi perdido?
- Qual região possui maior demanda?
- Qual rota é mais cara?
- O caminhão saiu com capacidade máxima?

---

# 🛠️ Tecnologias Utilizadas

## Banco de Dados
- MySQL
- PostgreSQL
- BigQuery

## Business Intelligence
- Power BI
- Looker Studio
- Excel

## Linguagens
- SQL
- Python
- DAX

---

# 📦 Estrutura de Dados

## Clientes
Informações cadastrais e histórico de compras.

## Produtos
Controle de categorias:
- Gelo em Cubo
- Gelo Britado
- Gelo em Barra

## Produção
- Controle industrial
- Custos operacionais
- Consumo energético

## Vendas
- Faturamento
- Ticket médio
- Volume vendido

## Logística
- Entregas
- Frota
- Custos de transporte

---

# 📊 Indicadores Monitorados

✅ Produção diária  
✅ Consumo energético  
✅ Faturamento mensal  
✅ Vendas por região  
✅ Performance logística  
✅ Controle de desperdício  
✅ Custos operacionais  
✅ Rentabilidade por cliente  

---

# 🚀 Benefícios do Projeto

- Redução de desperdícios
- Maior eficiência operacional
- Decisões baseadas em dados
- Redução de custos logísticos
- Escalabilidade operacional
- Maior controle financeiro

---

# 🔐 Visão Estratégica

O projeto busca transformar a distribuição de gelo em uma operação orientada por dados, utilizando Business Intelligence como ferramenta central de crescimento e escalabilidade.

---

# 👨‍💻 Desenvolvido por

## [Nome da Empresa]

Distribuição Inteligente • Logística • Business Intelligence

---

# 📄 Licença

Este projeto é destinado para fins empresariais, acadêmicos e operacionais.

Todos os direitos reservados.
```
